...
def housing_env(temp,rhum,livestock_type):
    temp = np.array(temp)
    ...
    if livestock_type.lower() == 'dairy':
        ...
        u_in[temp>=12.5] = 0.38
        rhum_in = rhum - 10
    elif livestock_type.lower() == 'broiler':
        ...
        u_in[temp>=12.5] = 0.43 
        rhum_in = rhum
    elif livestock_type.lower() == 'layer':
...
def barn_env(temp,wind):
    ...
    t_barn[temp>temp_idx] = temp + tempdiff
...
'''if production_system == 'industrial':
    T_in, u_in, RH = housing_env(temp_data,rhum_data,livestock)
...
evap = water_evap_a(temp=T_in,rhum=RH,u=u_in)*1000
evap_slat = water_evap_a(temp=T_in,rhum=RH,u=u_in)*f_slat*1000
evap_pit = water_evap_a(temp=T_in,rhum=RH,u=0.6)*1000
## housing resistance; s/m
R_star_slat = resistance_water_air(temp=T_in,rhum=RH,evap_flux=water_evap_a(temp=T_in,rhum=RH,u=u_in))
## correction factor for indoor resistance R_star
F_rcorret = 1.0
R_star_pit = F_rcorret * resistance_water_air(temp=T_in,rhum=RH,evap_flux=water_evap_a(temp=T_in,rhum=RH,u=0.6))
## temperature for calculating equilibrium
Total_Temp = T_in
...
mois_coeff = (-np.log(1.01-(RH/100))/(0.0000534*(T_in+273.15)))**(1/1.41)
...
Henry_constant = (161500/(Total_Temp + 273.15)) * np.exp(-10380/(Total_Temp + 273.15))
## dissociation constant of NH4+
k_NH4 = 5.67e-10*np.exp(-6286*(1/(Total_Temp + 273.15)-1/298.15))
...
## test code:
#print("The mean urea hydrolysis rate is: "+str(np.nanmean(daily_urea_hydro_rate)))
#print("The mean Henry's constant is: "+str(np.nanmean(Henry_constant)))
#print("The mean slat resistance is: "+str(np.nanmean(R_star_slat)))
#print("The mean pit resistance is: "+str(np.nanmean(R_star_pit)))
print("The mean daily urea is: "+str(np.nanmean(durea)))
print("The mean manure water is: "+str(np.nanmean(manure_wc)))
print("The mean daily urine is: "+str(np.nanmean(durine)))'''


'''evap = evap.values
evap_slat = evap_slat.values
evap_pit = evap_pit.values
R_star_slat = R_star_slat.values
R_star_pit = R_star_pit.values
excret_N = excret_N.values
durine_N = durine_N.values
durea = durea.values
dmanure_N = dmanure_N.values
dmanure = dmanure.values
mois_coeff = mois_coeff.values'''
