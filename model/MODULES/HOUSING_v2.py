...
if production_system == 'industrial':
    T_in, u_in, RH_in = housing_env(temp_data,rhum_data,livestock)
...
'''class HOUSING_MODULES:
    ...
    def __init__(self,array_shape):
        ...
        self.manure_pool_pit_to_storage = np.zeros(mtrx)
        self.avail_N_pool_pit_to_storage = np.zeros(mtrx)
        self.resist_N_pool_pit_to_storage = np.zeros(mtrx)
        self.unavail_N_pool_pit_to_storage = np.zeros(mtrx)
        self.TAN_pool_pit_to_storage = np.zeros(mtrx)
        self.Total_water_pool_pit_to_storage = np.zeros(mtrx)

        self.manure_pool_to_storage = np.zeros(mtrx)
        self.urea_pool_to_storage = np.zeros(mtrx)
        self.avail_N_pool_to_storage = np.zeros(mtrx)
        self.resist_N_pool_to_storage = np.zeros(mtrx)
        self.unavail_N_pool_to_storage = np.zeros(mtrx)
        self.TAN_pool_to_storage = np.zeros(mtrx)
        self.Total_water_pool_to_storage = np.zeros(mtrx)
        self.NH3_flux_from_barn = np.zeros(mtrx)
    ...
    evap = water_evap_a(temp=T_in,rhum=RH_in,u=u_in)*1000
    evap_slat = water_evap_a(temp=T_in,rhum=RH_in,u=u_in)*fslat*1000
    evap_pit = water_evap_a(temp=T_in,rhum=RH_in,u=0.6)*1000
    ## housing resistance; s/m
    R_star = 
    R_star_slat = resistance_water_air(temp=T_in,rhum=RH_in,evap_flux=water_evap_a(temp=T_in,rhum=RH_in,u=u_in))
    ## correction factor for indoor resistance R_star
    F_rcorret = 1.0
    R_star_pit = F_rcorret * resistance_water_air(temp=T_in,rhum=RH_in,evap_flux=water_evap_a(temp=T_in,rhum=RH_in,u=0.6))

    ## animal waste N info
    # excret_N = livestock_N_info(livestock_type='PIG', region='NA')
    F_Ncorrect = 1.0
    excret_N = F_Ncorrect*excretN_info/farming_area   ## unit: kg N per m^2 per year
    excret_N = xr_to_np(excret_N)
    # excret_N = F_Ncorrect*excretN_info*(1000/365)/farming_area         ## g N per m^2 per day
    ## animal waste info
    ...
    if livestock.lower()=="broiler":
        ...
        daily_urea_hydro_rate = urea_hydrolysis_rate(temp=T_in,delta_t=timestep)
        ## daily decomposition rate of available and resistant N components
        daily_Na_decomp_rate, daily_Nr_decomp_rate = N_pools_decomp_rate(temp=T_in, delta_t=timestep)
    ...
    def slat_pit_housing_sim(self,start_day_idx,end_day_idx,f_slat,f_gap):
        for dd in np.arange(start_day_idx,end_day_idx-1):
            ...
            self.urea_pool_pit[dd+1] = self.urea_pool_pit[dd] * (1 - self.daily_urea_hydro_rate[dd+1]) + \
                                    self.urea[dd+1]*f_gap + self.urea_pool_slat[dd]*(1-self.daily_urea_hydro_rate[dd+1])
            ...
            self.TAN_pool_pit[dd+1][TAN_pit_idx>0] = self.TAN_pit_idx[TAN_pit_idx>0]+self.TAN_prod_pit[dd+1][TAN_pit_idx>0]+\
            ...
            self.TAN_amount_slat[dd+1][Total_water_pool_slat[dd+1]==0] = 0
            self.TAN_amount_slat[dd+1][Total_water_pool_slat[dd+1]!=0] = self.TAN_pool_slat[dd+1][self.Total_water_pool_slat[dd+1]!=0]/\
                                                        self.Total_water_pool_slat[dd+1][self.Total_water_pool_slat[dd+1]!=0]
            self.TAN_amount_pit[dd+1][Total_water_pool_pit[dd+1]==0] = 0
            self.TAN_amount_pit[dd+1][Total_water_pool_pit[dd+1]!=0] = self.TAN_pool_pit[dd+1][self.Total_water_pool_pit[dd+1]!=0]/\
    ...
    def barn_sim(start_day_idx,end_day_idx):
        for dd in np.arange(start_day_idx,end_day_idx-1):
            ...
            ## water pool 
            self.water_idx = self.Total_water_pool[dd] + self.urine[dd+1] + self.manure_initwc_slat[dd+1] - \
            ...
            self.TAN_amount[dd+1][Total_water_pool[dd+1]!=0] = self.TAN_pool[dd+1][self.Total_water_pool[dd+1]!=0]/\
    ...
    def poultry_housing_sim(start_day_idx,end_day_idx):
        ...
        for dd in np.arange(start_day_idx,end_day_idx-1):
            ...
            emiss_idx = (NH3_gas_ug[dd+1]*3600*timestep/R_ab) - TAN_pool_ug[dd+1]
            modelled_emiss[dd+1][emiss_idx>0] = TAN_pool_ug[dd+1][emiss_idx>0]
            modelled_emiss[dd+1][emiss_idx<0] = NH3_gas_ug[dd+1][emiss_idx<0]*3600*timestep/R_star_p
...
def cleaning_pit(day_idx):
    ...
    self.manure_pool_pit_to_storage[day_idx] = manure_pool_pit[day_idx]+manure_pool_slat[day_idx]
    self.avail_N_pool_pit_to_storage[day_idx] = avail_N_pool_pit[day_idx]
    self.resist_N_pool_pit_to_storage[day_idx] = resist_N_pool_pit[day_idx]
    self.unavail_N_pool_pit_to_storage[day_idx] = unavail_N_pool_pit[day_idx]
    self.TAN_pool_pit_to_storage[day_idx] = TAN_pool_pit[day_idx]+urea_pool_pit[day_idx]
    self.Total_water_pool_pit_to_storage[day_idx] = Total_water_pool_pit[day_idx]
...
def cleaning_barn(day_idx):
    ## pools: from housing to MMS
    self.manure_pool_to_storage[day_idx] = manure_pool[day_idx]
    self.urea_pool_to_storage[day_idx] = (1 - daily_urea_hydro_rate[day_idx])*urea_pool[day_idx]
    self.avail_N_pool_to_storage[day_idx] = (1 - daily_Na_decomp_rate[day_idx])*avail_N_pool[day_idx]
    self.resist_N_pool_to_storage[day_idx] = (1 - daily_Nr_decomp_rate[day_idx])*resist_N_pool[day_idx]
    self.unavail_N_pool_to_storage[day_idx] = unavail_N_pool[day_idx]
    self.TAN_pool_to_storage[day_idx] = TAN_pool[[day_idx]]-NH3_flux[day_idx]
    self.Total_water_pool_to_storage[day_idx] = Total_water_pool[day_idx]
    self.NH3_flux_from_barn[day_idx] = NH3_flux[day_idx]
...
######################################
## HOUSING MODULE: slat-pit houses
######################################
class SLAT_PIT_HOUSING:
    ##################################
    ## define prognostic variables
    ##################################
    def __init__(self,array_shape):
    ...
    ## correction factor for indoor resistance R_star
    F_rcorret = 1.0
    R_star_pit = F_rcorret * resistance_water_air(temp=T_in,rhum=RH_in,evap_flux=evap_pit/1000)
    ...
    cleaning_freq = int(90 * nhours / timestep)
    ...
    def housing_to_storage_init(self):
        self.manure_pool_pit_to_storage[:] = 0.0
        self.avail_N_pool_pit_to_storage[:] = 0.0
        self.resist_N_pool_pit_to_storage[:] = 0.0
        self.unavail_N_pool_pit_to_storage[:] = 0.0
    ...
    def sim_main(self,start_idx,end_idx,cleaning_frequency):
        print()
        print()
        self.housing_init()
        self.housing_to_storage_init()
        for dd in np.arange(start_idx,end_idx,cleaning_frequency):
            if dd + cleaning_frequency < end_idx:
                ...
                self.cleaning_pit(dd+cleaning_frequency+1)
            else:
                self.slat_pit_housing_sim(dd,end_idx,self.fslat,self.fgap)
        return
        
####################################################
## HOUSING MODULE: barn houses (intermediate)
####################################################
class BARN_HOUSING:
    ####################################
    ## define prognostic variables
    ...
    evap = water_evap_a(temp=T_in,rhum=RH_in,u=u_in)*1000
    ## housing resistance; s/m
    R_star = resistance_water_air(temp=T_in,rhum=RH_in,evap_flux=evap/1000)
    ## animal waste N info
    # excret_N = livestock_N_info(livestock_type='PIG', region='NA')
    F_Ncorrect = 1.0
    excret_N = F_Ncorrect*excretN_info/farming_area   ## unit: kg N per m^2 per year
    ...
    def cleaning_barn(self,day_idx):
        ## pools: from housing to MMS
    ...
    def sim_main(self,start_idx,end_idx,cleaning_frequency):
        ...
        for dd in np.arange(start_idx,end_idx,cleaning_frequency):
            if dd + cleaning_frequency < end_idx:
                ...
                self.cleaning_barn(dd+cleaning_frequency+1)
            else:
                self.barn_housing_sim(dd,end_idx)
        return

######################################
## HOUSING MODULE: poultry houses
######################################
class POULTRY_HOUSING:
    ####################################
    ## define prognostic variables
    ####################################
    def __init__(self,array_shape):
        ...
        ## pools left after housing; transfer to storage
        self.manure_pool_to_storage = np.zeros(mtrx)
        self.UA_pool_to_storage = np.zeros(mtrx)
        self.avail_N_pool_to_storage = np.zeros(mtrx)
        self.resist_N_pool_to_storage = np.zeros(mtrx)
        self.unavail_N_pool_to_storage = np.zeros(mtrx)
        self.TAN_pool_to_storage = np.zeros(mtrx)
    ...
    excret_N = F_Ncorrect*excretN_info/farming_area   ## unit: kg N per m^2 per year
    excret_N = xr_to_np(excret_N)
    N_birds_UAN = excret_N * 1000/365 * f_uan
    manure = N_birds_UAN/f_excretn
    ## pH and H+ ions concentration; poultry manure pH set to 8.5
    pH = 8.5
    cc_H = np.float(10**(-pH))
    ## mositure equilirium, mositure content of manure
    mois_coeff = (-np.log(1.01-(RH_in/100))/(0.0000534*(T_in+273.15)))**(1/1.41)
    ...
    def poultry_housing_sim(self,start_day_idx,end_day_idx):
        for dd in np.arange(start_day_idx,end_day_idx-1):
            ...
            manure_water[dd+1][mois_coeff[dd+1]>=5] = manure_pool[dd+1][mois_coeff[dd+1]>=5] * mois_coeff[dd+1][mois_coeff[dd+1]>=5] / 100
            manure_water[dd+1][mois_coeff[dd+1]<5] = 0
            Total_water_pool[dd+1] = manure_water[dd+1]
            ...
            emiss_idx = (NH3_gas_ug[dd+1]*3600*timestep/R_ab) - TAN_pool_ug[dd+1]
            modelled_emiss[dd+1][emiss_idx>0] = TAN_pool_ug[dd+1][emiss_idx>0]
            modelled_emiss[dd+1][emiss_idx<0] = NH3_gas_ug[dd+1][emiss_idx<0]*3600*timestep/R_star
